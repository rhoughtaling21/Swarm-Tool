...
public class DiagonalLines extends AbstractStrategy {
	...
	@Override
	public void logic(Board board, SwarmAgent agent) {
		int totalCells = board.getTotalCellCount();
		int row = board.calculateAgentRow(agent);
		int col = board.calculateAgentColumn(agent);
		...
		if(newColor != oldColor) {
			board.getPersistenceLayer()[row][col].reset();
		}
		//If the cell does not need changed, darken the purple
		else {
			board.getPersistenceLayer()[row][col].shiftState();
		}
	}
}